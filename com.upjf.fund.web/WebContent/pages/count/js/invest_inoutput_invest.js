...
function initData(){
	...
	$("#tableInvestInOutInvestList").jqGrid({
		url : basePath+"report/investInOutInvestList",
		datatype : "json",
		mtype : "POST",
		height : 450,
		autowidth : true,
		...
		rowNum : 10,
		rowList : [ 10, 20, 30 ],
		pager : "#pagerInvestInOutInvestList",
	...
	$("#tableInvestInOutInvestList").jqGrid('setLabel','rn', '序号');
    
	$("#tableInvestInOutInvestList").jqGrid("navGrid", "#pagerInvestInOutInvestList", {
		edit : false,
		add : false,
		del : false,
		search : false,
...
function beforeSelectRow(){
	$("#tableInvestInOutInvestList").jqGrid('resetSelection');
    return(true);
}
...
function getInvestDetail(){
	//获取多选到的id集合
	var ids = $("#tableInvestInOutInvestList").jqGrid("getGridParam", "selarrrow");
	if(ids == undefined || ids == "" || ids.length > 1){
		layer.msg("请选择一条数据！",{icon: 2});
		return false;
	}
	//由id获得对应数据行
	var rowData = $("#tableInvestInOutInvestList").jqGrid('getRowData', ids);
...
function showInvestDetail(pid){
	if(hasLoadTable == 1){
		$("#tableInvestInOutInvestDetailList").jqGrid('setGridParam',{  
            datatype:'json',  
            postData:{pid:pid},
        }).trigger("reloadGrid");
	}else{
		$.jgrid.defaults.styleUI = "Bootstrap";
		$("#tableInvestInOutInvestDetailList").jqGrid({
			...
			rowNum : 10,
			rowList : [ 10, 20, 30 ],
			pager : "#pagerInvestInOutInvestDetailList",
		...
		$("#tableInvestInOutInvestDetailList").jqGrid('setLabel','rn', '序号');
	    
		$("#tableInvestInOutInvestDetailList").jqGrid("navGrid", "#pagerInvestInOutInvestDetailList", {
			edit : false,
			add : false,
			del : false,
			search : false,
