...
function initData(){
	...
	$("#tableInvestInOutRepayList").jqGrid({
		url : basePath+"report/investInOutRepayList",
		datatype : "json",
		mtype : "POST",
		height : 450,
		autowidth : true,
		...
		rowNum : 10,
		rowList : [ 10, 20, 30 ],
		pager : "#pagerInvestInOutRepayList",
	...
	$("#tableInvestInOutRepayList").jqGrid('setLabel','rn', '序号');
    
	$("#tableInvestInOutRepayList").jqGrid("navGrid", "#pagerInvestInOutRepayList", {
		edit : false,
		add : false,
		del : false,
		search : false,
...
function beforeSelectRow(){
	$("#tableInvestInOutRepayList").jqGrid('resetSelection');
    return(true);
}
...
function getRepayDetail(){
	//获取多选到的id集合
	var ids = $("#tableInvestInOutRepayList").jqGrid("getGridParam", "selarrrow");
	if(ids == undefined || ids == "" || ids.length > 1){
		layer.msg("请选择一条数据！",{icon: 2});
		return false;
	}
	//由id获得对应数据行
	var rowData = $("#tableInvestInOutRepayList").jqGrid('getRowData', ids);
...
function showRepayDetail(subjectReturnPid){
	if(hasLoadTable == 1){
		$("#tableInvestInOutRepayDetailList").jqGrid('setGridParam',{  
            datatype:'json',  
            postData:{'subjectReturnPid':subjectReturnPid}, 
        }).trigger("reloadGrid");
	}else{
	    $.jgrid.defaults.styleUI = "Bootstrap";
		$("#tableInvestInOutRepayDetailList").jqGrid({
			...
			rowNum : 10,
			rowList : [ 10, 20, 30 ],
			pager : "#pagerInvestInOutRepayDetailList",
		...
		$("#tableInvestInOutRepayDetailList").jqGrid('setLabel','rn', '序号');
	    
		$("#tableInvestInOutRepayDetailList").jqGrid("navGrid", "#pagerInvestInOutRepayDetailList", {
			edit : false,
			add : false,
			del : false,
			search : false,
