...
<title><c:if test="${investType eq 1 }">投资主体</c:if><c:if test="${investType eq 2 }">投资人</c:if>投资汇总</title>
            ...
            <div class="bg-border">
                ...
                <div class="search-box">
	                        ...
	                        <select name="contributiveId" lay-verify="" lay-search="">
	                        	...
	                        	<c:if test="${investType eq 1 }">
		                        	<c:forEach var="investSub" items="${investSubCompanyList}">
		                            	<option value="${investSub.invest_subject_id }" <c:if test="${investSub.invest_subject_id eq contributiveId }">selected="selected"</c:if>>${investSub.invest_name }</option>
		                           	</c:forEach>
	                        	</c:if>
	                        	<c:if test="${investType eq 2 }">
	                        		<c:forEach var="investor" items="${investorCompanyList}">
		                            	<option value="${investor.invest_subject_id }" <c:if test="${investor.invest_subject_id eq contributiveId }">selected="selected"</c:if>>${investor.invest_name }</option>
		                           	</c:forEach>
	                        	</c:if>
	                        </select>
                        </div>
                    ...
                    <div class="search-box_div search_box_div_left">
                        <button type="button"  class="search-btn search-btn-chaxu" onclick="searchList('#tableInvestInOutInvestList')">
...
<!--正文-->
        <div class="bg-white">
            <div class="bg-border">
                <div class="title-head">
                    ...
                    <span><c:if test="${investType eq 1 }">投资主体</c:if><c:if test="${investType eq 2 }">投资人</c:if>投资汇总</span>
                ...
                <div class="tabled_one">
                    <!--表格-->
                    <table id="tableInvestInOutInvestList"></table>
                    <!--底部翻页按钮栏 -->
		         	<div id="pagerInvestInOutInvestList"></div>
    ...
    <div id="Item_Inoutput_Out" class="add_Makeuup">
        ...
        <div class="main_edge">
            ...
            <div class="tabled_datum_940">
	            <div class="tabled_one">
	                <!--表格-->
	                <table id="tableInvestInOutInvestDetailList"></table>
	                <!--底部翻页按钮栏 -->
	       			<div id="pagerInvestInOutInvestDetailList"></div>
	            </div>
            </div>
        </div>
    </div>
	...
	<script type="text/javascript" src="<%=path %>/pages/count/js/invest_inoutput_invest.js"></script>
</body>
</html>