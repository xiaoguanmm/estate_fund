...
$ (function () {
		...
		layui.use('form',function(){
			...
			// accessType=1表示从新增股东入口过来,让其新增项目
			if($('#accessType').val() == '1') {
				$('#busPrjInfoSelect_Id').val($('#businessPrjIdFromAddStock').val());
				$('#businessPrjInfoId').val($('#businessPrjIdFromAddStock').val());
				$('#busPrjInfoSelect_Id').attr("disabled",true);
			}
...
//校验预算名称是否与上一条一致
function cheackBudget(pid,budgetName) {
	var isTheSame = false;
	$.ajax({
	    url: basePath+"/projectManageController/cheackBudget",
	    type: "POST",
	    data:{pid:pid,budgetName:budgetName},
	    dataType:"json",
	    async: false,
		success : function(data) {
			var resultMsg=data["msg"];
...
function saveBudget() {
	$('#prjId').val($('#pid').val());
	//校验预算名称是否跟上条相同
	var projectBudgetPid = $('#projectBudgetPid').val();
	var budgetName = $('#budgetName').val();
	var result = cheackBudget(projectBudgetPid,budgetName);
	if(result){
		$('#projectBudgetPid').val('');
	}
	
	$.ajax({
	    url: basePath+"/projectManageController/saveBudgetData",
	    type: "POST",
	    data:$('#project_budgetForm').serialize(),
	    dataType:"json",
	    async: false,
		success : function(data) {
			var resultMsg=data["msg"];
			...
			if(successCode == 's'){
				...
				var projectBudgetId=data["projectBudgetId"];
				$('#projectBudgetPid').val(projectBudgetId);
...
function savaProjectBaseInfo() {
	$('#busPrjInfoSelect_Id').attr("disabled",false);
	$.ajax({
