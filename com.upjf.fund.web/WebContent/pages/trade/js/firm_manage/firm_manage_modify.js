...
$ (function () {
	    ...
	    var corBasePidVal = $('#corBasePid').val();
	    
	    if(corBasePidVal != null){
		...
		layui.use('form',function(){
			var form =layui.form;
			form.on('select(update_corpQualityFilter)', function(data){
			...
			form.on('select(updateForm_bankFilter)', function(data){
				var bankVal =  data.value;
		...
		//启用上传
		 layui.use('upload', function(){
			  ...
			  var uploadInst = upload.render({
			    ...
			    ,before: function(obj){ 
			    	...
			    	$(this)[0].data.corPid = $('#corBasePid').val();
		 ...
		 $('.datum_upload2').click(function () {
				var newCorPidVal = $('#corBasePid').val();
				if(newCorPidVal == ''){
		...
		//企业基本信息保存按钮
		$('#update_corBaseSubmit').click(function () {
			var arr = ['update_corName','update_orgCodeCert','update_businessLicenceCode'];
			if(cheackAndShowWarns(arr)){
				focusWarnInput(arr);
				return false;
			}
			saveUpdateBaseCorInfo();
		});
		
      
		//企业基础信息必填项校验
		$("#update_corName,#update_orgCodeCert,#update_businessLicenceCode").blur( function () {
		...
		//企业信息-银行管理保存按钮
		$('#update_bankSubmit').click(function () {
			var corBasePidVal = $('#corBasePid').val();
			if(corBasePidVal == ''){
				layer.msg("请先保存企业基本信息",{icon: 7,time: 1000});
				return false;
			}else{
				$('#corpId').val(corBasePidVal);
			}
			saveCorBankInfo();
		});
					...
					for(var i = 0;i<dataInfoList.length;i++){  
                        ...
                        for(var j = 0;j<datum_imgList.length;j++){  
                            ...
                            if(corDataType == datumId){
        			    		...
        			    		$('#'+datumId+'_del').addClass('datum_btnactive');
        				    	$('#'+datumId+'_priview').addClass('datum_btnactive');
                            }
                        }
                    }
				}
...
//保存修改的企业基本信息
function saveUpdateBaseCorInfo() {
	$.ajax({
	    url: basePath+"/corporationController/updateBase",
	    type: "POST",
	    data:$('#updateBaseCorInfoForm').serialize(),
	    dataType:"json",
	    async: false,
		success : function(data) {
			var resultMsg=data["msg"]
			var success=data["success"]
			if(success == "s"){
				$('#corporationName').val($('#update_corName').val());
...
function saveCorBankInfo() {
	$.ajax({
	    ...
	    type: "POST",
	    data:$('#update_bankFormId').serialize(),
	    dataType:"json",
	    async: false,
		success : function(data) {
			var resultMsg=data["msg"]
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(successCode == 's'){
				...
				$('#update_corporationName').val($('#update_corName').val());
