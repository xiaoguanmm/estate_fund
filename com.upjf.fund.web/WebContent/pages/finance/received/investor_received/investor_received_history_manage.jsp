...
<title>投资人回款历史记录列表</title>
    <%@ include file="../../../../common/common.jsp"%>
    <link href="${basePath}pages/finance/invest/css/invest.css" rel="stylesheet">
    <script type="text/javascript">
		$(function(){
			var url = basePath+"finance/queryInvestorHistoryPaybackList?prjId=${project_id}&receiverId=${receiver_id}&contributiveId=${contributive_id}&stockholderId=${stockholder_id}&receivedType=3"
			var colNames = ["回款时间","回款账号","回款本金(万元)","回款利润(万元)","项目名称",
			                "付款公司","回款状态","回款备注","回款录入人",
			                "回款录入时间","pid","receiver_bank_id","pay_bank_id","pay_account","received_status"];
			var colModel = [
						    ...
						    {name: "contributive_name",index: "contributive_name", sortable :false,formatter:function(cellvalue, options, rowObject){
						    	return '<span>${contributive_name}</span>';
						    }},
						    ...
						    {name: "pay_account",index: "pay_account", sortable :false,hidden:true},
        ...
        <div class="bg-white">
            <div class="bg-border">
                <div class="title-head">
                    ...
                    <span>回款列表 </span>
                    <span class="title-head_red">&nbsp;&nbsp;${receiver_name}</span>
                </div>
                <div class="top_btn">
               ...
               <div id="payback_operation" class="add_Makeuup">
                <input type="hidden" id="uploadFileUrl" value="${basePath}finance/uploadInvestorPaybackAccessory">
                <input type="hidden" id="showFileListUrl" value="${basePath}finance/queryInvestorPaybackAccessoryList">
                <input type="hidden" id="delPaybackUrl" value="${basePath}finance/delInvestorPaybackRecord">
                <input type="hidden" id="exportUrl" value="${basePath}finance/exportInvestorHistoryPaybackData">
                <input type="hidden" id="projectStatus" value="${project_status}">
                <input type="hidden" id="received_status" value="${received_status}">
                <form id="investForm" action="${basePath}finance/addInvestSubjectPaybackRecord" method="post">
                	<!-- 付款公司 -->
                	<input type="hidden" class="hide_field" id="corporationInfoId" value="${contributive_id}">
                	<input type="hidden" class="hide_field" id="pid" name="pid">
                	<input type="hidden" class="hide_field" name="prjId" value="${project_id}">
                	<!-- 收款公司 -->
                	<input type="hidden" class="hide_field" id="corpId" name="receiverId" value="${receiver_id}">
                	<input type="hidden" class="hide_field" name="contributiveId" value="${contributive_id}">
                	<!-- 所属股东 -->
                	<input type="hidden" class="hide_field" id="stockholderId" name="stockholderId" value="${stockholder_id}">
                	<!-- 回款类别 -->
                	<input type="hidden" class="hide_field" id="receivedType" name="receivedType" value="3">
                    <div class="main_edge">
                                ...
                                <div class="fillbox_inline">
                                    ...
                                    <div class="fillin-inputlay layui-form form-group">
                                            <select id="receiverId" disabled="disabled" class="form-control" lay-verify="" lay-search="" lay-filter="receiverId">
                                                <option value="${receiver_id}">${receiver_name}</option>
                                            </select>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="fillbox_inline">
                                    ...
                                    <div class="fillin-inputlay layui-form form-group">
                                            <select id="contributiveId" disabled="disabled" class="form-control" lay-verify="" lay-search="" lay-filter="contributiveId">
                                                <option value="${contributive_id}">${contributive_name}</option>
                                            </select>
                                    </div>
                                </div>
                                <div class="fillbox_inline">
