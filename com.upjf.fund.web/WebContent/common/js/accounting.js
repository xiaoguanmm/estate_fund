...
(function(root, undefined) {
	...
	var unformat = lib.unformat = lib.parse = function(value, decimal) {
		...
		var regex = new RegExp("[^0-9-" + decimal + "]", [ "g" ]), unformatted = parseFloat(("" + value)
				.replace(/\((.*)\)/, "-$1").replace(regex, '').replace(decimal,
						'.'));
	...
	var formatNumber = lib.formatNumber = lib.format = function(number,
		...
		var opts = defaults((isObject(precision) ? precision : {
			...
			thousand : thousand,
			decimal : decimal
		}), lib.settings.number), usePrecision = checkPrecision(opts.precision), negative = number < 0 ? "-"
				: "", base = parseInt(toFixed(Math.abs(number || 0),
				usePrecision), 10)
	...
	var formatMoney = lib.formatMoney = function(number, symbol, precision,
		...
		var opts = defaults((isObject(symbol) ? symbol : {
			...
			decimal : decimal,
			format : format
		}), lib.settings.currency), formats = checkCurrencyFormat(opts.format), useFormat = number > 0 ? formats.pos
				: number < 0 ? formats.neg : formats.zero;
	...
	lib.formatColumn = function(list, symbol, precision, thousand, decimal,
		...
		var opts = defaults((isObject(symbol) ? symbol : {
			...
			decimal : decimal,
			format : format
		}), lib.settings.currency), formats = checkCurrencyFormat(opts.format), padAfterSymbol = formats.pos
				.indexOf("%s") < formats.pos.indexOf("%v") ? true : false, maxLength = 0, formatted = map(
				list,
				function(val, i) {
					if (isArray(val)) {
						...
						val = unformat(val);
						var useFormat = val > 0 ? formats.pos
								: val < 0 ? formats.neg : formats.zero, fVal = useFormat
								.replace('%s', opts.symbol).replace(
										'%v',
